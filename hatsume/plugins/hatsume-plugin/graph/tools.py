...
async def create_daily_timer(
    ...
    结束时间规则：如果用户明确指定结束时间，必须使用该时间；如果用户未指定，必须结合
    任务内容和当前聊天上下文自行选择合理的有限 end_at，即使任务没有明显的自然期限也不得
    仅因此向用户追问。工具成功创建任务后，必须在自然回复中明确告诉用户实际传入的结束时间。
